...
### [Optional] Step 1 
Find the .vscode folder present in your workspace and make sure that a launch.json and tasks.json file are present.
If not please create one through the VS Code debugger, by following the guidelines the debugger displays when clicked on.

![VSCode Debugger Screenshot](./img/launch-configuration.png)

VS Code will try to automatically detect your debug environment, but if this fails, you will have to choose it manually:

![VSCode Debug Environment Screenshot](./img/debug-environments.png)
...
Once those files are created copy and paste the following configurations:
...
The general understanding of what these parameters and what the task.json and launch.json represent in VS Code can be found in the Dapr doc[How-to: VSCode Multiple Dapr Apps Debugging](https://docs.dapr.io/developing-applications/ides/vscode/vscode-how-to-debug-multiple-dapr-apps/)

The reason task.json contains both `daprd-debug-node` and `daprd-debug-python` both which happen to run the commands we had mentioned earlier in the tutorial.
...
These parameters choosen represent the needed parameters to match the commands but also contains additional information to both specify and avoid port collisions that could happen with multiple apps being started at the same time through Daprd which currently does not automatically look for open ports if one of its parameters is missing a value.

### [Optional] Step 3

Now that you have all the configurations needed you will be able to find the compound launch configuration ready to launch in the VS Code debugger window.

![VSCode Debugger Screenshot](./img/vscode-launch-configuration.png)

Once you hit the run command setup a breakpoint and debug your application any way you want.
