...
- `job-scheduler.py`, responsible for scheduling, retrieving and deleting jobs.
- `job-service.py`, responsible for handling the triggered jobs.
...
```bash
dapr run --app-id job-service --app-port 6200 --dapr-http-port 6280 -- python job-service.py
```
