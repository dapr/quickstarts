...
In this sample, we'll create two microservices, one with an input binding and another with an output binding. We'll bind to Kafka and Event Hubs, but note that there are a myriad of components that Actions can bind to - see Actions components <INSERT_LINK_HERE>.  
...
with output bindings to Kafka, and another with input bindings. In the output bindings case, we won't need to explicitly send messages to Kafka, but rather will talk through our actions sidecar. Similarly in the input bindings case, we'll simply set up API endpoints that actions will call as Kafka gets messages. 
